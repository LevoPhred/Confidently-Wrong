...
var DEFAULT_GAME_STATE = {
  ...
  players: {
    ...
    // incremented each time a player lands exactly on the Hangar after a
    // full lap (see simulateHangarWager() in index.html). turnsTaken is a
    // simple counter of completed turns, shown on the Dispatch console.
    p1: { name:'', sprite:'sprite-heli-yellow',  tile:'', score:0, position:0, connected:false, missionSkipPending:false, lapsCompleted:0, turnsTaken:0 },
    p2: { name:'', sprite:'sprite-plane-yellow', tile:'', score:0, position:0, connected:false, missionSkipPending:false, lapsCompleted:0, turnsTaken:0 },
    p3: { name:'', sprite:'sprite-heli-blue',    tile:'', score:0, position:0, connected:false, missionSkipPending:false, lapsCompleted:0, turnsTaken:0 },
    p4: { name:'', sprite:'sprite-plane-green',  tile:'', score:0, position:0, connected:false, missionSkipPending:false, lapsCompleted:0, turnsTaken:0 },
    p5: { name:'', sprite:'sprite-heli-red',     tile:'', score:0, position:0, connected:false, missionSkipPending:false, lapsCompleted:0, turnsTaken:0 },
    p6: { name:'', sprite:'sprite-plane-red',    tile:'', score:0, position:0, connected:false, missionSkipPending:false, lapsCompleted:0, turnsTaken:0 },
    p7: { name:'', sprite:'sprite-heli-green',   tile:'', score:0, position:0, connected:false, missionSkipPending:false, lapsCompleted:0, turnsTaken:0 },
    p8: { name:'', sprite:'sprite-plane-blue',   tile:'', score:0, position:0, connected:false, missionSkipPending:false, lapsCompleted:0, turnsTaken:0 }
  },
...
function spinMoveIndicator(){
  var state = GameStore.getState();
  if (state.paused) return; // Dispatch has frozen the game for a discussion
...
function pickTieredQuestion(tier){
  var pool = QUESTION_BANK[tier];
  var categories = Object.keys(pool);
  var category = categories[Math.floor(Math.random() * categories.length)];
  var list = pool[category];
  var index = Math.floor(Math.random() * list.length);
  return { tier: tier, category: category, index: index };
}
...
function getTieredQuestion(tier, category, index){
  var pool = QUESTION_BANK[tier];
...
function resolveMission(correct){
  var state = GameStore.getState();
  if (state.paused) return; // Dispatch has frozen the game — a stale timeout call shouldn't sneak a resolution in
...
function submitMissionAnswer(optionIndex){
  ...
  if (state.paused) return; // Dispatch has frozen the game for a discussion
  if (state.phase !== 'RESOLVING_MISSION' || state.turn.subphase !== 'MISSION_ANSWERING') return;
  var q = getTieredQuestion(state.turn.missionTier, state.turn.missionCategory, state.turn.missionQuestionIndex);
  if (!q) return;
...
function submitRapidAnswer(optionIndex){
  ...
  if (state.paused) return; // Dispatch has frozen the game for a discussion
  if (state.phase !== 'RESOLVING_RAPID' || !state.turn.rapidBuzzedBy || state.turn.rapidResult) return;
  var q = getTieredQuestion(state.turn.rapidTier, state.turn.rapidCategory, state.turn.rapidQuestionIndex);
  if (!q) return;
