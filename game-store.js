...
function getRoomCodeFromURL(){
  ...
  // Switched from CW-4821 to a brand-new room: after dozens of test writes
  // across many incompatible versions of this file's state shape during
  // development, that old room may have accumulated a large amount of
  // leftover data. Connecting to a bloated room and downloading/parsing a
  // very large snapshot on connect could itself cause exactly the freeze
  // we were chasing — independent of any of this file's own code. Starting
  // clean sidesteps that possibility entirely.
  return params.get('room') || 'CW-6674';
}
...
// to shared/multiplayer mode once it succeeds. If the network is slow,
// filtered, or the CDN is unreachable (a real possibility on a base or
// classroom network), this gives up after 6 seconds rather than leaving
// the page in limbo — which is what happened when the SDK was loaded via
// a plain <script src> tag instead.
function loadFirebaseSDK(){
  ...
  var timeoutId = setTimeout(function(){
    if (settled) return;
    settled = true;
    console.warn('Firebase SDK did not load within 6s — staying in local mode.');
  }, 6000);
  ...
  loadScript('https://www.gstatic.com/firebasejs/10.13.2/firebase-app-compat.js')
    .then(function(){
      return loadScript('https://www.gstatic.com/firebasejs/10.13.2/firebase-database-compat.js');
    })
    .then(function(){
      if (settled) return; // timeout already fired, don't upgrade late
      settled = true;
